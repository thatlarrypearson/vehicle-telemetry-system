...
for vin in vins:
    ...
    obd_gear_study = generate_gear_study_data(f"{temporary_file_base_directory}/{vin}", vin)
    ...
    save_gear_study_data_to_csv(vin, output_file_name, obd_gear_study, force_save=False)
    ...
    df = pd.read_csv(f"{temporary_file_base_directory}/Gear-Study/Gear-Study-{vin}.csv", parse_dates=['iso_ts_pre', 'iso_ts_post', ])
