...
A Polynomial Multivariate Regression Function created by [Google Gemini](https://gemini.google.com), an AI agent.

Polynomial Multivariate Regression Function's arguments:

- ```csv_filepath``` (Path)
- ```degree``` (int >= 1 and <= 3)
- ```dependent_variable``` name

## [Google Gemini](https://gemini.google.com) Prompt

```text
write function to perform polynomial multivariate regression in python that takes a CSV file with headers, the degree and the dependent variable
```

## [Google Gemini](https://gemini.google.com) Show Thinking

```text
This script defines a Polynomial Multivariate Regression Function function called ```perform_polynomial_regression(csv_filepath, degree, dependent_variable)``` that:

Reads your CSV file using pandas.

Splits your data into independent (X) and dependent (y) variables. It also handles cases where non-numeric columns might be present in the independent variables by dropping them.

Splits the data into training and testing sets to evaluate the model's performance on unseen data.

Generates polynomial features for your independent variables up to the specified degree.

Fits a LinearRegression model to these polynomial features.

Calculates and prints the R-squared score on the test set, which indicates how well the model fits the data.

Returns the fitted model, the PolynomialFeatures transformer (useful for transforming new data before prediction), and the R-squared score.
